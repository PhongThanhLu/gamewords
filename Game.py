...
class WOFPlayer():
    def __init__(self, name):
        ...
        self.prizeMoney = 0      # Prize money initialized to 0
        self.prizes = []
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        list_guessing = [letter for letter in LETTERS if letter not in guessed]
        if self.prizeMoney < VOWEL_COST:
            list_guessing = [letter for letter in list_guessing if letter not in VOWELS]
        return list_guessing
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if self.smartCoinFlip():
            # Sort the possible letters by their frequency, most frequent last
            freq_sorted = sorted(possible_letters, key=lambda x: self.SORTED_FREQUENCIES.index(x))
            return freq_sorted[-1]  # Return the most frequent valid letter
        else:
